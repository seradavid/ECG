...
void plot_points(int x, int y, int cx, int cy)
{
...
{
	//int currentX = circle.centerX;
	int currentY = circle.radius;
	int d;

	d = 1 / 4 - circle.radius;

	int lim = circle.radius / sqrt(2);
	for (int currentX = 0; currentX <= lim; ++currentX)
	{
		plot_points(currentX, currentY, circle.centerX, circle.centerY);
